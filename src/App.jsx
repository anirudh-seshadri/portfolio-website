...
const detailedProjects = [
  ...
  fit: "contain",
  paragraphs: [
    "This project is an LED pattern controller built using an ESP32 microcontroller and a breadboard-based circuit. The system is designed to control multiple LEDs and display different lighting patterns such as blinking, chasing, bouncing, and sequential filling. A pushbutton is used as input to switch between these patterns, allowing the system to respond to user interaction in real time. Each LED is connected to a GPIO pin on the ESP32 through a current-limiting resistor, and all components share a common ground.",

    "On the hardware side, I assembled the circuit on a breadboard using multiple LEDs, resistors, jumper wires, and a pushbutton. The LEDs were wired to different GPIO pins on the ESP32, and each LED was connected in series with a resistor to prevent excessive current. The pushbutton was connected using an internal pull-up configuration, meaning one side of the button was connected to ground and the other to a GPIO input pin. When pressed, the button pulls the signal low, which is detected in the program.",

    "On the software side, I wrote embedded C++ code to control the LEDs and implement the different lighting patterns. The program uses an array of GPIO pins to manage multiple LEDs efficiently. Timing for the patterns is controlled using the millis() function instead of delay(), allowing the system to run continuously without blocking execution. I implemented a state-based system where each button press increments a mode variable, and a switch statement selects which LED pattern to execute based on the current mode.",

    "Each pattern function updates the LED outputs differently. For example, the blinking pattern toggles all LEDs on and off, the chase pattern turns on one LED at a time in sequence, the bounce pattern moves a single active LED back and forth across the array, and the fill pattern progressively turns LEDs on and off. Additional variables are used to track the current position, direction, and state of each pattern, allowing the animations to run smoothly over time.",

    "I also implemented button debouncing logic to ensure that each press is only registered once. This prevents unintended multiple inputs caused by mechanical noise in the button. The system continuously reads the button input, updates the current mode when a press is detected, and resets the pattern state variables to ensure correct behavior when switching between modes."
  ]
},
];
